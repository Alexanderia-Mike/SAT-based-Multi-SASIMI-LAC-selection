...
// // adding the sorting network to all the selection signals. 
// // each selection signal is repeated for DArea times.
// static void Miter_Add_Sorting_Network_Area_Encoded ( Abc_Ntk_t * pMiter, int oriPINum, std::vector<Abc_Obj_t *> &sortedSelectionSignals, const std::vector<LAC_t> & candLACs, const bool restrict_range, const int maximum_range );
...
// static double Calculate_Total_Reduced_Area ( char * SATResultFileName, const std::vector<LAC_t> & candLACs, const int nCnfVars, Abc_Ntk_t * pAppNtkDup );

// static void Alexanderia_ReplaceObj(Abc_Obj_t * pNodeOld, Abc_Obj_t * pNodeNew);

// static void Alexanderia_DeleteObj_rec(Abc_Obj_t * pObj);

// static void Range_Map ( std::vector<int> & rangeMappedList, const std::vector<int> & original_list, const int maximum_range );

// static void Apply_LAC ( LAC_t lac, Abc_Ntk_t * pAppNtk );
...
//     char * SATResultFileName = "intermediate-results/SAT_Problem_Out.qdimacs";
...
//     vector <LAC_t> nodeLACsDup;        // all the LACs
//     vector <LAC_t> candLACs;        // all the candidates (more concise)
//     vector <LAC_t> candLACsDup;        // all the candidates (more concise)
//     random_device rd;
...
//         Alexanderia_CollectAllLACsUnderER(oriSmlt, *pAppSmlt, bds, vMffcs, nodeLACs, nodeLACsDup, pAppNtkDup);
//     else
...
//         assert( nodeLACs[i].GetDArea() == nodeLACsDup[i].GetDArea() );
//     // debug end
//     */
...
//     SortCandLACs(nodeLACsDup, pAppSmlt->GetFrameNum(), candLACsDup);

//     /*
//     // debug begin
//     assert( candLACs.size() == candLACsDup.size() );
//     for ( int i = 0; i < candLACs.size(); ++i )
//     {
//         assert( candLACs[i].GetDArea() == candLACsDup[i].GetDArea() );
//         assert( candLACsDup[i].GetSS()->pNtk == candLACsDup[i].GetTS()->pNtk );
//     }
//     // debug end
//     */
...
//     if ( candLACs.size() == 0 )
//     {
//         cout << "no possible LAC exists!" << endl;
//         return;
//     }
...
//     {
//         candLACs.resize( 40 );
//         candLACsDup.resize( 40 );
//     }
//     // debug end

//     /*
//     for ( int j = 0; j < candLACsDup.size(); ++j )
//     {
//         assert( candLACsDup[j].GetSS()->pNtk == candLACsDup[j].GetTS()->pNtk );
//     }
//     */

//     // debug begin
//     cout << "the reduced area of all LAC candidates are listed as follows:" << endl;
//     // cout << "the network type is: " << candLACs[0].GetSS()->pNtk->ntkType << endl;
//     for ( int i = 0; i < candLACs.size(); ++i )
//     {
//         assert( candLACs[i].GetDArea() == candLACsDup[i].GetDArea() );
//         cout << candLACs[i].GetDArea() << " " << candLACs[i].GetDError() << " " << candLACs[i].GetFOM() << " ";
//         cout << "SS name: " << Abc_ObjName( candLACs[i].GetSS() ) << ", TS name: " << Abc_ObjName( candLACs[i].GetTS() ) << endl;
//     }
//     cout << endl;
//     // debug end
...
//     // Abc_NtkMapToSop( pAppNtkDup );

//     /*
//     // debug begin
//     cout << "after transforming to sop:" << endl;
//     cout << "the network type is: " << candLACs[0].GetSS()->pNtk->ntkType << endl;
//     for ( int i = 0; i < candLACs.size(); ++i )
//     {
//         cout << candLACs[i].GetDArea() << " " << candLACs[i].GetDError() << " " << candLACs[i].GetFOM() << " ";
...
//     if ( !CreateMuxedCNF( pAppNtk, candLACs, cnfFileName, threshold, pOriPIIDs, pMUXPIIDs, &nCnfVars ) )
//     {
//         cout << "no LAC can be applied!" << endl;
//         return;
//     }
//     // cout << "the number of cnf variables is " << nCnfVars << endl;
...
//     totalSavedArea = Calculate_Total_Reduced_Area( SATResultFileName, candLACsDup, nCnfVars, pAppNtkDup );
//     if ( totalSavedArea == -1 )
//         cout << "failed!" << endl;
//     else
//         cout << "the area saved is " << totalSavedArea << endl;
...
// bool SASIMI_Manager_t::CreateMuxedCNF ( IN Abc_Ntk_t * pMUXedNtk, IN std::vector <LAC_t> & candLACs, IN char * cnfFileName, int threshold[], int ** pOriPIIDs, int ** pMUXPIIDs, int * pNCnfVars )
// {
...
//     Miter_Add_Sorting_Network_Area_Encoded( pMiter, OriPINum, sortedSelectionSignals, candLACs, false, 20 );
...
//     {
//         cout << "------------- the least index has exceeds the boundary!" << endl;
//         return false;
//     }    
...
//     int i; // if true, continue the outer loop
//     Abc_Obj_t * pObj;
...
//         // debug begin
//         cout << "i = " << i << endl;
//         cout << "pOriNtk's size = " << pOriNtk->vObjs->nSize << endl;
//         cout << "pCopiedNtk's size = " << pCopiedNtk->vObjs->nSize << endl;
//         // debug end
//         */
...
// static double Calculate_Total_Reduced_Area ( char * SATResultFileName, const std::vector<LAC_t> & candLACsDup, const int nCnfVars, Abc_Ntk_t * pAppNtkDup )
// {
//     ifstream        ifStream;
...
//     string          trashBin2;
//     int             LACIndex;
//     const string    stringV( "V" );
//     const string    string0( "0" );
//     vector<int>     LACIndices;
//     double          originalArea;
//     double          reducedArea;
//     LAC_t           lac;
...
//     ifStream.open( SATResultFileName, ifstream::in );
//     if ( !ifStream.is_open() )
//     {
...
//         /*
//         // debug begin
//         cout << "iLine is " << iLine  << endl;
//         cout << "the string in isStream is " << isStream.str() << endl; 
//         cout << "LACIndex is " << LACIndex << endl;
//         cout << "LACIndices has size " << LACIndices.size() << endl;
//         // debug end
//         */
//         isStream.clear();
//     }
...
//         cout << "LACIndices[" << i << "]=" << LACIndices[i] << endl;
//     // debug end
//     */
...
//         cout << "LACIndices[" << i << "]=" << LACIndices[i] << endl;
//         // add up all the reduced area for activated LACs
//         // totalArea += candLACs[LACIndices[i]].GetDArea();
//         lac = candLACsDup[LACIndices[i]];
//         // Alexanderia_ReplaceObj( lac.GetTS()->pCopy, lac.GetSS()->pCopy );
//         Apply_LAC( lac, pAppNtkDup );
//     }
//     reducedArea = Abc_NtkGetMappedArea( pAppNtkDup );
//     cout << "original area = " << originalArea << " ";
//     cout << "reduced area = " << reducedArea;
...
//     if ( Abc_ObjFanoutNum( pTS ) == 0 || Abc_ObjFanoutNum( pSS ) == 0 )
//     {
//         return;
//     }
...
//         Alexanderia_ReplaceObj(pTS, pSS);
//     }
//     else {
...
//         Abc_Obj_t * pInv = Abc_NtkCreateNodeInv(pAppNtk, pSS);  // complemented...
//         Alexanderia_ReplaceObj(pTS, pInv);
//     }
// }
...
//     // remove the old node
//     Alexanderia_DeleteObj_rec( pNodeOld );
// }
...
//             Alexanderia_DeleteObj_rec( pObj );
//     Vec_PtrFree( vNodes );
// }

// static void Range_Map ( std::vector<int> & range_mapped_list, const std::vector<int> & original_list, const int maximum_range )
// {
