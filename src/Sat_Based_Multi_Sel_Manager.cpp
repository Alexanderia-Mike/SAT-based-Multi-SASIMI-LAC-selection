...
SBMSM_t::print_candLAC()
{
    ...
    for ( int i = 0; i < candLACs.size(); ++i )
    {
        ...
        std::cout << candLACs[i].GetDArea() << " " << candLACs[i].GetDError() << " " << candLACs[i].GetFOM() << " ";
...
SBMSM_t::create_miter_finalize()
{
    ...
    miter_add_sorting_network_area_encoded( sortedSelectionSignals, true, 40 );
...
SBMSM_t::transform_miter_to_cnf ()
{
    ...
    assign_PIIDs( miterCnfData, true );
